...
2. **`tahuti grades` is redundant:** *(Correction/Erratum: ManageBac **does** calculate and expose official running overall term grades and category weights in the `#sidebar_info` `Task Category Averages` drawer of `/student/classes/<id>/core_tasks`. The previous `tahuti` implementation relied on an unweighted heuristic `_compute_expected_grade` averaging Highcharts points because the crawler author overlooked the sidebar drawer. Individual task grades are also visible in `tahuti list` and `tahuti view`.)*
